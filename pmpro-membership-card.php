...
// Require the Apple Wallet Pass class.
require( plugin_dir_path( __FILE__ ) . 'includes/pkpass/PKPass.php' );
...
function pmpro_membership_card_generate_apple_pass( $user = NULL ) {
	...
	if ( empty( $_REQUEST['pmpro_membership_add_to_wallet'] ) ) {
		...
		<div id="pmpro_membership_add_apple_wallet">
			...
			?>
			<a href="<?php echo esc_url( add_query_arg( $wallet_query_args ) ); ?>" id="pmpro_membership_card_add_to_apple"><img src="<?php echo plugin_dir_url( __FILE__ ) . 'includes/images/add-to-apple-wallet.svg'; ?>" alt="Add to Apple Pay" /></a>
		</div>
		<?php
		return ob_get_clean();
	} else {
		...
		$certificate_url = 'https://someurl/Certificates.p12'; /// Make sure this file is uploaded to the right place.
		...
		$assets = apply_filters( 'pmpro_membership_card_apple_wallet_assets', array(
			'logo' => plugin_dir_path( __FILE__ ) . 'includes/images/logo.png',
			'thumbnail' => plugin_dir_path( __FILE__ ) . 'includes/images/thumbnail.png',
			'icon' => plugin_dir_path( __FILE__ ) . 'includes/images/icon.png',
			'icon@2x' => plugin_dir_path( __FILE__ ) . 'includes/images/icon@2x.png'
		) );
